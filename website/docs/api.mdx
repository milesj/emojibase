...
objects. It requires a file path relative to the `emojibase-data` package as the 1st argument and an
option object of options as the 2rd argument.

```ts
import { fetchFromCDN, flattenEmojiData } from 'emojibase';

fetchFromCDN('ja/compact.json', { version: '2.1.3' }).then(flattenEmojiData);
```
...
- `local` (`boolean`) - Cache the response in local storage instead of session storage.
